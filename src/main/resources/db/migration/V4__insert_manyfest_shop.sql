WITH existing_context AS (
    ...
    WHERE name = 'skate' AND insert_user_id = 1 AND active = true
     ...
     existing_shop AS (
         SELECT id
         FROM shop
         WHERE name = 'manyfestbmx' AND country = 'PL' AND context_id = (SELECT id FROM existing_context)
     ),
    inserted_shop AS (
        INSERT INTO shop (name, country, context_id, insert_user_id, created_date, updated_date, active)
            SELECT 'manyfestbmx', 'PL', id, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true
